...
reader = csv.reader(open(path_file_csv, encoding="utf8"))
...
for i in range(0, len(translations)):
    if translations[i] == 'id':
        translations[i] = 'in'
    if translations[i] == '(Backup text)' or translations[i] == 'en':
        continue
    android_dir = current_path + "/app/src/main/res/values"
    if translations[i] != "en-us" : 
        android_dir += "-"+translations[i][:2].lower()
    ...
    xml_file = open(android_dir + "/strings.xml", "w", encoding="utf8")
